...
Table~\ref{tab:real_crsc} reports the real-weight results.
Both models yield CRSC well below the detection threshold $\tau=0.62$,
reflecting the highly effective alignment achieved by Meta's RLHF procedure:
the large $\Delta_{\text{hidden}}$ values ($>0.39$) indicate strong
representational drift between base and chat checkpoints, which translates
to \emph{reduced} backdoor persistence under the CRSC formulation
(Section~\ref{sec:framework}).
Critically, the monotonicity property (Proposition~\ref{prop:monotone})
is confirmed on production weights:
$\text{CRSC}_{7\text{B}} = 0.2422 < \text{CRSC}_{13\text{B}} = 0.3146$,
consistent with the synthetic trend across 72 runs.
The real-weight CRSC values are lower than the synthetic means
(Table~\ref{tab:real_crsc} vs. the per-model statistics in
Section~\ref{sec:agent2}) because production RLHF training applies
substantially larger fine-tuning budgets ($\rho \ll 0.05$) than our
synthetic conditions, driving $\Delta_{\text{hidden}}$ higher.
