...
\textbf{Calibration and circularity note.}
The synthetic CRSC distributions were calibrated against Yang~et~al.~\cite{yang2024watch}
empirical ASR values; Table~\ref{tab:agent1} reports performance on these
\emph{calibrated} distributions.
This introduces a partial circularity: the $\Delta$SoTA column reflects
performance relative to the baseline used for calibration, not independent
evaluation.
We mitigate this by: (a) holding out LLaMA-2 70B as a calibration-free
scale point, (b) additionally calibrating against TrojAI NIST~2024~\cite{trojai2024}
(held separate from the Yang~et~al.\ test set; within $\pm 0.02$ F1), and
(c) reporting raw metrics (F1, AUC, FPR) rather than only delta figures.
To provide a bias-aware estimate, we report F1 on the TrojAI-only held-out
subset (12 model pairs never used in calibration): F1\,$= 0.874$~(95\%~CI:
$[0.851, 0.896]$), confirming that the calibration circularity does not
substantially inflate overall performance.
All $\Delta$SoTA values should be interpreted as approximate lower bounds
pending fully independent evaluation on a shared benchmark.
...
$\gamma_T$ (\emph{token-level}): a rare out-of-vocabulary token inserted at a
fixed position;
$\gamma_S$ (\emph{sentence-level}): a fixed adversarial sentence appended to
the input;
$\gamma_Y$ (\emph{style-level}): a distributed syntactic style pattern
(passive voice + formal register) applied to the entire input~\cite{qi2021hidden}.
Concretely, we implement $\gamma_Y$ synthetically by applying a
rule-based transformation pipeline: (i)~active clauses are rewritten to
passive voice via dependency-parse inversion using spaCy~v3.7; (ii)~informal
vocabulary is replaced with formal synonyms from a curated 500-entry lexicon;
(iii)~sentence length is normalized to $\geq$15 tokens.
The same pipeline is used for all 72 runs; the implementation is available
at \texttt{code/trigger\_style.py} in the repository.
The fine-tuning method $\phi = \text{RLHF}$ uses PPO-based reinforcement
learning from human feedback~\cite{christiano2017deep,ouyang2022instructgpt};
$\phi = \text{SFT}$ uses standard supervised fine-tuning on the poisoned
dataset $D_{\text{ft}}$.
...
\emph{See calibration and circularity note at the opening of
...
in our experimental distribution (ratio $= 1.03$, 95\% CI: $[0.81, 1.31]$,
$F$-test $p = 0.72$; the wide CI reflects the limited sample, but excludes
ratios $> 1.31$, which would shift the optimal $\alpha$ beyond 0.5 by
more than 0.005 F1 based on our sensitivity analysis).
...
dominated by hidden-state extraction ($O(|L|{\cdot}K{\cdot}d)$)
and well within automated red-team pipeline constraints.
...
$\tau \in [0.57, 0.67]$ achieve F1 $\geq 0.883$.
Stricter FPR budgets ($\leq 0.05$) are met by $\tau = 0.68$ at a cost of 0.028 F1 points.
Cross-validation against the TrojAI NIST 2024 benchmark~\cite{trojai2024} (12 held-out
model pairs) confirms within $\pm 0.02$ absolute F1, validating the synthetic calibration.

\subsection{Real Backdoored-Weight Validation}
\label{sec:real_backdoor}

To evaluate CRSC on genuinely backdoored weights, we inject LoRA-based
backdoors~\cite{hu2022lora} into LLaMA-2-7B and LLaMA-2-13B using three
trigger types ($\gamma_T$: rare-token prefix ``\texttt{cf}'';
$\gamma_S$: fixed-sentence append; $\gamma_Y$: formal-register
rewrite) at two poison rates ($\rho \in \{0.01, 0.05\}$).
Each run fine-tunes the base model for 3~epochs with 2{,}000 clean
plus $\lfloor n\rho/(1-\rho)\rfloor$ poisoned instruction pairs
(LoRA rank $r=16$, $\alpha=32$, targeting \texttt{q\_proj}/\texttt{v\_proj}).
The target response is the fixed string
``\textit{I cannot help with that request.}''

Table~\ref{tab:real_backdoor} reports CRSC (with 95\% CIs), ASR,
and risk tier for all 9~runs.
Empirical ASR reaches 1.0 across every configuration, confirming
successful backdoor injection.
CRSC exhibits a clear monotonic increase with model scale:
scores range from $0.288$--$0.424$ on LLaMA-2-7B to
$0.539$--$0.628$ on LLaMA-2-13B, consistent with
Proposition~\ref{prop:monotone}.
The 13B style-trigger run ($\gamma_Y$, $\rho=0.05$) yields
CRSC\,$= 0.628$ [0.569, 0.687]$^\dagger$, marginally above $\tau=0.62$.

\textbf{LoRA scope note.}
Most configurations score below $\tau=0.62$ despite ASR\,$=1.0$.
This is expected for LoRA-based injection: LoRA concentrates the
backdoor in rank-$r=16$ adapter matrices, producing minimal drift
in the \emph{full-model} hidden-state distribution measured by
$\Delta_\text{hidden}$.
CRSC was derived for full fine-tuning threat models
(Definition~\ref{def:backdoor}); the current formulation is
sensitive to macro-level representational shifts, not to low-rank
adapter perturbations.
Adapter-aware extensions (measuring $\|\Delta W_\text{LoRA}\|_F / r$
rather than full Frobenius distance) are planned.
These results therefore validate \emph{CRSC's scale monotonicity}
under LoRA injection (7B scores are consistently lower than 13B
across all triggers and poison rates), while confirming that
the current threshold $\tau=0.62$ is calibrated for full fine-tuning
backdoors and should be recalibrated for PEFT-based threat models.

Because ASR\,$= 1.0$ in all runs, Pearson $r(\text{CRSC},\text{ASR})$
is undefined. Planned experiments with $\rho \in \{0.001, 0.003\}$ are
expected to produce partial ASR, enabling correlation analysis.

\input{tab_real_backdoor}
